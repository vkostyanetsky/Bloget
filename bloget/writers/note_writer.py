...
def _write_note(
    ...
    file_text = _get_file_text(note, previous_note, next_note, metadata)
...
def _get_template_parameters(
    ...
    if previous_note:
        result["previous_note_path"] = _get_note_page_path(previous_note)
        result["previous_note_title"] = previous_note.title
        result["hotkey_ctrl_left_url"] = _get_note_page_url(
    ...
    if next_note:
        result["next_note_path"] = _get_note_page_path(next_note)
        result["next_note_title"] = next_note.title
        result["hotkey_ctrl_right_url"] = _get_note_page_url(
...
def _get_note_page_path(note: page_reader.BlogPage) -> str:
    """
    Returns a path to a note.

    For instance, notes/note_name
    """

    return f"{constants.NOTES_FOLDER_NAME}/{note.folder_name}"
